...
### 인증
- [x] **Q-01** `index.html` 접속 → `auth.html` 리디렉트 확인
- [x] **Q-02** Google OAuth 로그인 → 칸반 보드 진입 확인
- [x] **Q-03** GitHub OAuth 로그인 → 칸반 보드 진입 확인
- [x] **Q-04** 이메일 회원가입 → 확인 이메일 안내 표시 확인
- [x] **Q-05** 이메일 로그인 → 칸반 보드 진입 확인
- [x] **Q-06** 로그아웃 → `auth.html` 리디렉트 확인

### 칸반 기능
- [x] **Q-07** 카드 추가 → Supabase Table Editor에서 DB 저장 확인
- [x] **Q-08** 카드 편집 → DB 반영 확인
- [x] **Q-09** 카드 삭제 → DB 삭제 확인
- [x] **Q-10** 드래그 → 컬럼 이동 + DB `col` 업데이트 확인
- [x] **Q-11** Done 드롭 → 컨페티 이펙트 확인
- [x] **Q-12** 검색어 입력 → 카드 필터링 확인
- [x] **Q-13** 다크모드 토글 → 새로고침 후 유지 확인
- [x] **Q-14** 다른 계정 로그인 → 카드 분리 확인 (RLS 검증)
- [x] **Q-15** 모바일 375px 세로 스택 레이아웃 확인
